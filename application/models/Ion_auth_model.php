...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, email, id_user, password, active, last_login')
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.'id_group'.' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description')
		                ->where($this->tables['users_groups'].'.'.'id_user', $id)
		                ->join($this->tables['groups'], $this->tables['users_groups'].'.'.'id_group'.'='.$this->tables['groups'].'.id_group')
	...
	public function add_to_group($group_id, $user_id=false)
	{
		...
		if ($this->db->where(array( 'id_group' => (int)$group_id, 'id_user' => (int)$user_id))->get($this->tables['users_groups'])->num_rows()) return false;

		if ($return = $this->db->insert($this->tables['users_groups'], array( 'id_group' => (int)$group_id, 'id_user' => (int)$user_id)))
		{
	...
	public function remove_from_group($group_ids=false, $user_id=false)
	{
		...
		{
			if ($return = $this->db->delete($this->tables['users_groups'], array('id_user' => (int)$user_id))) {
	...
	public function group($id = NULL)
	{
		...
		if (isset($id))
		{
			$this->db->where($this->tables['groups'].'.id_group', $id);
		}
